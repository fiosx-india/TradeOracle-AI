...
class PluginRegistry:
    ...
    def initialize_all(self) -> None:
        ...
        Plugin initialization is isolated so that a failure in one plugin does
        not prevent the remaining plugins from being initialized.
        """
        with self._lock:
            plugins = tuple(self._plugins.items())
        ...
        for name, plugin in plugins:
            ...
            if not callable(method):
                logger.debug(
                    "Plugin does not implement initialize().",
                    extra={"plugin": name}
                )
                continue
    ...
    def _run_analysis_for_symbol(
        ...
        """
        Execute the complete AI analysis workflow for a single symbol.

        This method orchestrates the symbol-level analysis pipeline using the
        existing helper methods. It invokes registered extensions before and
        after analysis, builds the resulting AI insight, updates the internal
        caches, and returns the completed insight.
        ...
        Returns:
            The completed ``AIInsight`` instance.
        """
        context: Mapping[str, Any] = {
            "symbol": symbol,
            "market": market,
        ...
        self._invoke_extensions_before(context)

        insight = self._build_ai_insight(
            symbol=symbol,
            market=market,
            news=news,
            signal=signal,
        ...
        self._invoke_extensions_after(insight)
    ...
    def analyze(self) -> Dict[str, AIInsight]:
        ...
        This method acts as the public orchestration entry point for the
        AIMarketOracle. It coordinates the existing TradeOracle components and
        delegates all analysis work to the previously implemented helper
        methods.

        Returns:
            A mapping of trading symbol to the corresponding ``AIInsight``.

        Raises:
            RuntimeError:
                Propagated if the underlying TradeOracle analysis inputs cannot
                be collected.
        """
        self._logger.info("Starting AI market analysis.")

        market_data, market_reports, news_report = self._collect_analysis_inputs()

        insights: Dict[str, AIInsight] = {}

        for symbol, index_data in market_data.items():
            self._logger.debug(
                "Analyzing symbol.",
                extra={"symbol": symbol},
            )
